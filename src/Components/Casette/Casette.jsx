    ...
    <div style={Styles.casette}>
      <div className={Styles.casette_img}>
        ...
        <div className={Styles.cassette_body}>
          ...
          <img src="https://res.cloudinary.com/dhgfsshq4/image/upload/v1677821241/posua2023/button_left_gkghbz.webp" alt="" onClick={()=>{year===2021?setYear(parseInt(year)-2):setYear(parseInt(year)-1); setCdRotate(cdRotate - 180)}} className={`${Styles.button_left} ${year<=2016?Styles.disabled:''}`} />
          <img src="https://res.cloudinary.com/dhgfsshq4/image/upload/v1677821241/posua2023/button_left_gkghbz.webp" alt="" onClick={()=>{year===2019?setYear(parseInt(year)+2):setYear(parseInt(year)+1); setCdRotate(cdRotate + 180)}} className={`${Styles.button_right} ${year>=2022?Styles.disabled:''}`} />
        </div>
      </div>
    </div>
  );
